...
describe('AssemblyLine', () => {
  ...
  const getStage = n =>
    getDeepTextContents(getAllByTestId('stage')[n]).filter(
      e => !stages.includes(e),
    );
  const pressOn = (row, col, event = 'press') =>
    fireEvent(
      within(getAllByTestId('stage')[row]).getAllByTestId('item')[col],
      event,
    );
  beforeEach(() => {
    ({debug, queryByTestId, queryAllByTestId, getAllByTestId, getByTestId} =
  ...
  describe('adding assembly items', () => {
    describe('when "goldenrod" is typed into "add-item"', () => {
      ...
      describe('when pressed on "add-new-item"', () => {
        ...
        it(
          'should respond to item submission, ' +
        ...
        it(
          'should have a single item in the first stage ' +
